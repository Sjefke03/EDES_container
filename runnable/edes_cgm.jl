...
#
# FILE STRUCTURE:
#   SECTION 1: Variables and Constants (all magic numbers defined here)
#   SECTION 2: Function Definitions
#   SECTION 3: Simulations and Results
...
# ─── ODE problem setup ─────────────────────────────────────────────────────────
u0_ggut = 0.0                    # initial gut glucose
u0_irem = 0.0                    # initial remote insulin
ode_tspan_start = 0.0
ode_tspan_end = 240.0
u0 = [u0_ggut, Gb, Ib, u0_irem]
tspan = (ode_tspan_start, ode_tspan_end)
...
color_line = 1
color_scatter = 2
fig_output_dir = @__DIR__
fig_fit_name = "fig_cgm_fit.png"
fig_pla_name = "fig_cgm_pla.png"

# ─── PLA (Profile Likelihood Analysis) settings ───────────────────────────────
pla_k1_min, pla_k1_max = 1e-5, 0.02
pla_k1_N = 800
pla_k5_min, pla_k5_max = 0.001, 0.2
pla_k5_N = 800
pla_k6_min, pla_k6_max = 0.5, 20.0
pla_k6_N = 400

pla_ranges = [
    range(pla_k1_min, pla_k1_max, length=pla_k1_N),
    range(pla_k5_min, pla_k5_max, length=pla_k5_N),
    range(pla_k6_min, pla_k6_max, length=pla_k6_N),
]

pla_lb = [1e-6, 1e-6, 1e-6]
pla_ub = [1.0, 0.5, 10.0]

pla_param_names = ["k1", "k5", "k6"]
pla_fig_positions = [(1,1), (1,2), (2,1)]
pla_ylim_max = 5.0
chi2_alpha = 0.95
chi2_df = 1
color_hline = 3
...
# ─── Loss function (glucose only) ─────────────────────────────────────────────
const first_error_cgm = Ref(true)
function loss_CGM(θ, (problem, constants, data))
    glucose_data    = data[2,:]
    data_timepoints = data[1,:]
    penalty = sum(abs2, θ[1:n_free_params]) * penalty_l2_weight + penalty_offset

    p = construct_parameters(θ, constants)
    local pred
    try
        pred = solve(problem, ode_solver, p=p, saveat=data_timepoints, save_idxs=ode_save_idxs,
...
function generate_initial_guesses(n_guesses, lower_bounds, upper_bounds; selection_method::SelectionMethod = SelectAll())
    initial_guesses = QuasiMonteCarlo.sample(n_guesses, lower_bounds, upper_bounds, lhs_sampling)
    return evaluate_and_select(initial_guesses, selection_method)
end

# ─── PLA infrastructure ───────────────────────────────────────────────────────
struct PLAResult
    likelihood_values::Vector{Float64}
    parameter_values::Vector{Float64}
    optim_index::Int
    other_parameter_values::Vector
end
...
function likelihood_profile!(ax, pla_result::PLAResult, param_index, param_name)
    ...
    lines!(ax, parameter_values, likelihood_values, color=Makie.wong_colors()[color_line])
end
...
initial_guess = generate_initial_guesses(n_initial_guesses, lb, ub)
...
println("CGM estimation: $(length(results))/$n_initial_guesses runs succeeded")
if isempty(results)
...
# ─── Generate solution from best fit ──────────────────────────────────────────
solution = solve(prob, p=construct_parameters(final, constants), u0=[u0_ggut, CGM_data[2,1], fasting_ins, u0_irem])

# ─── Figure: Model fit ────────────────────────────────────────────────────────
fig_fit = let f = Figure(size=(fig_fit_width, fig_fit_height))
    ...
    lines!(ax_g_gut,    solution.t, solution[1,:], color=Makie.wong_colors()[color_line])
    lines!(ax_g_plasma, solution.t, solution[2,:], color=Makie.wong_colors()[color_line])
    lines!(ax_i_plasma, solution.t, solution[3,:], color=Makie.wong_colors()[color_line])
    lines!(ax_i_int,    solution.t, solution[4,:], color=Makie.wong_colors()[color_line])

    band!(ax_g_plasma, solution.t, solution[2,:] .+ final[sigma_g_idx], solution[2,:] .- final[sigma_g_idx])
    band!(ax_i_plasma, solution.t, solution[3,:] .+ final[sigma_i_idx],   solution[3,:] .- final[sigma_i_idx])

    scatter!(ax_g_plasma, CGM_data[1,:], CGM_data[2,:], color=Makie.wong_colors()[color_scatter])
    f
end
save(joinpath(fig_output_dir, fig_fit_name), fig_fit)

println("Final estimated parameters (CGM fit): ", final)

# ─── Run Profile Likelihood Analysis (PLA) ───────────────────────────────────
pla_results = [
    run_pla(i, pla_ranges[i], final[i], loss_known_sigma_CGM, final[1:n_free_params],
            pla_lb, pla_ub,
            (prob, constants, CGM_data, final[sigma_g_idx], final[sigma_i_idx]))
    for i in 1:length(pla_param_names)
]

# ─── Figure: Profile Likelihood Analysis ─────────────────────────────────────
fig_pla = let f = Figure()
    for (idx, (pos, param_name)) in enumerate(zip(pla_fig_positions, pla_param_names))
        ax = Axis(getindex(f, pos...), xlabel=param_name, ylabel="ΔL", title="Profile Likelihood for $param_name")
        likelihood_profile!(ax, pla_results[idx], idx, param_name)
        scatter!(ax, [final[idx]], [0], color=Makie.wong_colors()[color_scatter])
        chi2_threshold = quantile(Chisq(chi2_df), chi2_alpha)
        hlines!(ax, [chi2_threshold], linestyle=:dash, color=Makie.wong_colors()[color_hline])
        ylims!(ax, (0.0, pla_ylim_max))
    end
    f
end
save(joinpath(fig_output_dir, fig_pla_name), fig_pla)

println("Done. Figures saved: $fig_fit_name, $fig_pla_name")
