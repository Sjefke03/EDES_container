# edes_ogtt_3param.jl
# OGTT scenario: estimate k1, k5, k6 with k8 fixed from plasma glucose + insulin data.
# Run from the repo root: julia runnable/edes_ogtt_3param.jl
...
# ─── Loss function (glucose + insulin, θ = [k1,k5,k6,σ_g,σ_i]) ──────────────
...
save(joinpath(@__DIR__, "fig_ogtt3_fit.png"), fig_fit)
...
pla_results = [
    run_pla(i, ranges[i], final[i], loss_known_sigma, final[1:3],
            [0.0, 0.0, 0.0], [1.0, 0.5, 10.0],
            (prob, constants, data, final[end-1], final[end]))
    for i in 1:3
]
...
save(joinpath(@__DIR__, "fig_ogtt3_pla.png"), fig_pla)

println("Done. Figures saved: fig_ogtt3_fit.png, fig_ogtt3_pla.png")
